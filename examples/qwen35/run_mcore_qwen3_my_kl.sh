...
export PYTHONPATH=${MEGATRON_PATCH_PATH}:${MEGATRON_PATCH_PATH}/backends/megatron/Megatron-LM-260226:$PYTHONPATH
...
OUTPUT_BASEPATH=${27}
PATCH_TOKENIZER_PATH=${28}
BASE_MODEL_CHECKPOINT_PATH=${29}
KL_LOSS_WEIGHT=${30}
KL_LOSS_TEMPERATURE=${31}
### OTHERS ###
...
if [ $SFT = true ]; then
    ...
    PREFIX="finetune-kl-mcore-qwen3-next-${MODEL_SIZE}-lr-${LR}-minlr-${MIN_LR}-kl-${KL_LOSS_WEIGHT}-bs-${BATCH_SIZE}-gbs-${GLOBAL_BATCH_SIZE}-seqlen-${SEQ_LEN}"
    sft_options=" \
         --eod-mask-loss \
         --train-mode finetune"
else
    ...
    PREFIX="pretrain-kl-mcore-qwen3-next-${MODEL_SIZE}-lr-${LR}-minlr-${MIN_LR}-kl-${KL_LOSS_WEIGHT}-bs-${BATCH_SIZE}-gbs-${GLOBAL_BATCH_SIZE}-seqlen-${SEQ_LEN}"
    sft_options=" \
        --train-mode pretrain"
fi
...
if [ ${MP_DATASET_TYPE} = "raw" ]; then
    ...
    dataset_options=" \
        ...
        --patch-tokenizer-path ${PATCH_TOKENIZER_PATH} \
        --split 99,1,0 \
...
if [ -d "${PRETRAIN_CHECKPOINT_PATH}" ]; then
    ...
    find -L ${PRETRAIN_CHECKPOINT_PATH} -maxdepth 1 -type f -name "merges.txt" -print0 | xargs -0 -r cp -t ${SAVED_PRETRAIN_CHECKPOINT_PATH}
fi

if [ "${BASE_MODEL_CHECKPOINT_PATH}" = "${PRETRAIN_CHECKPOINT_PATH}" ]; then
    echo "WARNING: BASE_MODEL_CHECKPOINT_PATH is the same as PRETRAIN_CHECKPOINT_PATH."
    echo "WARNING: This is usually only for smoke test. For real KL distillation please override BASE_MODEL_CHECKPOINT_PATH."
fi
...
megatron_options="  \
        ...
        --pipeline-model-parallel-layout ${PARALLEL_LAYOUT} \
        ...
        --apply-layernorm-1p \
        --enable-kl-distill \
        --base-model-load ${BASE_MODEL_CHECKPOINT_PATH} \
        --base-model-ckpt-format torch_dist \
        --kl-loss-weight ${KL_LOSS_WEIGHT} \
        --kl-loss-temperature ${KL_LOSS_TEMPERATURE}
        "

run_cmd="torchrun $DISTRIBUTED_ARGS pretrain_qwen3_next_my.py
